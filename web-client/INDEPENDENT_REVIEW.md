...
The following fixes from the priority list above have been implemented and committed (commit `adc41fd`):

| # | Area | Fix | Status |
|---|------|-----|--------|
| 1 | §3.2 | `encodeURIComponent()` on all waveform/signal names in 6 endpoint paths in `api.ts` | ✅ Done |
| 2 | §4.6 | `drawMultiBitWaveform` now uses `value.type \|\| value.valueType \|\| value.value_type` triple fallback | ✅ Done |
| 3 | §4.2 | `lodLevel` parameter added to `RenderCache.generateKey`, `get`, `set`, `has` | ✅ Done |
| 4 | §5.1+§5.2 | OPFS write lock (per-path serialization), atomic tmp+move, QuotaExceededError catch+GC+retry | ✅ Done |
| 5 | §3.1 | `fetchWithTimeout()` with AbortController (30s default, 5s health check) on all requests | ✅ Done |
| 6 | §5.4+§5.12 | `getSignalIdManager` promise-dedup; `saveMetadata` serialized chain + 200ms debounce | ✅ Done |
| 7 | §2.3+§2.6+§2.8 | Heartbeat timeout sends error+cancel; INITIALIZE disposes old provider; DISPOSE resets all state; pre-fetch cancellation check before WASM fetch | ✅ Done |
| 8 | §6.1+§6.2+§6.11 | `prevPropsRef` synced synchronously; Effect C applies `enableOpfs`/`enablePrefetch` dynamically | ✅ Done |
| 9 | §1.10 | Permanent rAF loop | 🟡 Intentional — continuous rAF needed for smooth mouse line tracking |
| 10 | §5.3 | epoch field in signalIdManager + render cache keys | ✅ Done |

**Additional fixes** (second batch, commit `42078e7`):
| — | §1.1 | SignalList async loading race — cancelled flag | ✅ Done |
| — | §1.10 | rAF loop — reverted to continuous loop; mouse line needs smooth per-frame tracking | 🟡 Design trade-off |
| — | §4.5 | O(n·m) large-group detection → O(n) with index map (both drawing files) | ✅ Done |
| — | §5.3 | epoch in signalIdManager; `getEpoch()` for cache key consumers | ✅ Done |
| — | §6.4 | `addNavigationEntry` uses `activeTabRef` instead of closure; removed `setTimeout` at 4 call sites | ✅ Done |
| — | §6.7 | `signalDisplayFormatsRef` synced synchronously in state setter | ✅ Done |
| — | §6.9 | `buildWasmSignals` result cached via ref, keyed on signal list hash | ✅ Done |

### Known Issues Not Yet Addressed

| # | Area | Issue | Severity |
|---|------|-------|----------|
| 4.1 | Render | Duplicate rendering code in `waveformRenderer.ts` vs `waveformDrawing.ts` — behavioral divergence in `getSignalLevel` | High |
| 4.4 | Render | No tile-boundary continuity check — false double-line artifacts at seams | Medium |
| 4.7 | Render | Memory estimation in `RenderCache` ~10× too low (uses 8 bytes/segment, reality ~80+) | High |
| 5.7 | Cache | OPFS reader worker loads entire file into memory | Medium |
| 5.10 | Cache | OPFS GC not wired to storage pressure watchdog | Medium |
| 5.11 | Cache | Source file content copy not verified | Low |
| 1.2 | UI | Language menu active highlight permanently broken | Medium |
| 1.6 | Render | No `ctx.save()`/`ctx.restore()` around canvas state modifications | Medium |
| 1.9 | UI | Duplicate `zoomIn`/`zoomOut` in `viewport.ts` and `zoomHelpers.ts` | Medium |
| 2.10 | Worker | WASM callbacks use `new Function()` — fragile under CSP | Medium |
| 3.4 | Server | `binaryRequest` error swallowed — callers can't distinguish error types | Medium |
| 3.5 | Server | `checkKdbChanged` returns `{changed:false}` on API failure | Medium |
| 3.6 | Server | `response.json()` success-path try/catch (applied to `request()`, not `binaryRequest()`) | Medium |
| 3.8 | Server | No retry logic despite comment claiming it | Low |
| 3.9 | Server | Large KDB decompression may exhaust memory | High |
| 4.11 | Render | Two overlapping cache classes (`RenderCache` + `RenderChunkCache`) with ~200MB combined budget | Medium |
| 5.9 | Cache | `writeOpfsBinary` lacks atomicity | Medium |
