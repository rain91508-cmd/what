...
- **信号値展開表示**：ソースコードで現在のカーソル時間の信号値を直接表示、多進数表示をサポート
...
- **テーブルビュー**：特定の時間範囲内の信号値を表形式で表示、多進数表示、メタデータフィルタリング、ページネーションをサポート
...
**ドライバー追跡：**
1. コード内のシグナル名をクリック
2. ポップアップメニューで「Find Drivers」を選択
...
**信号値展開表示：**
- コード行の左側にある展開アイコン（▶）をクリックすると、現在のカーソル時間におけるその行のすべての信号値を展開して表示
- 展開後、信号名、現在値、ビット幅などの情報が表示されます
- 多進数表示（バイナリ、16進数など）をサポート、波形ウィンドウの表示形式設定を自動的に継承
- 再度展開アイコンをクリックすると表示を折りたたむ
- 展開状態はタブ切り替え時に自動的に保存・復元されます
...
**ビュー操作：**
- **ズーム**：左マウスボタンで上にドラッグして拡大、下にドラッグして縮小；またはツールバーの +/- ボタンを使用
- **パン**：ルーラーエリアをドラッグ
- **エリアズーム**：左マウスボタンで左右にドラッグしてエリアを選択
...
テーブルビューは、特定の時間範囲内の信号値を表形式で表示し、信号状態の確認と分析に適しています：

**テーブルビューの作成：**
- ツールバーの「+」ボタンをクリックし、「Table」を選択して新しいテーブルビューを作成
- 波形ウィンドウがアクティブな状態でテーブルビューを作成すると、自動的に以下を継承します：
  - 現在の波形ウィンドウの時間範囲（View Start / View End）
  - 展開されたグループ内のすべての信号
  - 各信号の表示形式（Radix）
  - 信号プレフィックス設定

**信号管理：**
- 設計ブラウザからテーブルビューにシグナルをドラッグ
- 設計ブラウザのシグナルをダブルクリックしてテーブルに追加
- 列ヘッダーの「×」ボタンをクリックしてシグナルを削除
- 列ヘッダーをドラッグしてシグナルの順序を変更

**時間範囲設定：**
- ツールバーの「Start」と「Span」入力ボックスで時間範囲を設定
- 「Apply」ボタンをクリックして新しい時間範囲を適用し、データを取得
- ページネーションをサポート、「Previous」と「Next」ボタンでページ移動
- 「Continue」ボタンをクリックしてより多くのデータを取得（存在する場合）

**表示形式設定：**
- 列ヘッダーのドロップダウン矢印をクリックして形式選択メニューを開く
- バイナリ（BIN）、オクタル（OCT）、デシマル（DEC）、ヘキサデシマル（HEX）表示をサポート
- 各信号列は独立して設定可能

**メタデータフィルタリング：**
- 信号値の特徴によるフィルタリング：X状態、Z状態、混合状態、遷移、トグル
- 複数のフィルタ条件は「OR」論理で結合

**列フィルタリング：**
- 列ヘッダーの入力ボックスにフィルタ条件を入力
- 16進数値フィルタリングをサポート（例：`0x1a`）
...
3. 保存内容には以下が含まれます：
   ...
   - 開いているすべてのテーブルビュー（シグナルリストと時間範囲を含む）
   - ブックマーク
